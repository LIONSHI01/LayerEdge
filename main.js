...
const logger = {
  ...
  error: (message, value = "", error = null) =>
  ...
  verbose: (message, value = "") =>
    logger.verbose && logger.log("verbose", message, value),
...
async function run() {
  ...
  try {
    ...
    logger.info(
      "配置已加载",
      `钱包数量: ${wallets.length}, 代理数量: ${proxies.length}`
    );
